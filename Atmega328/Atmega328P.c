...
uint8_t live_count(void){ uint8_t c=0; for(uint8_t i=0;i<MAX_ENEMIES;i++) if(enem[i].act) c++; return c; }

// retorna 1 si victoria, 0 si derrota
uint8_t run_level(uint8_t level){
    // Dificultad:
    uint16_t enemy_step_ms   = (level==1)? 650 : (level==2)? 550 : 450;  // paso de enemigos
    uint16_t pshot_step_ms   = 60; // paso de disparos
    uint16_t spawn_gap_ms    = (level==1)? 800: (level==2)? 600: 400;  // spawns
    uint8_t  target_enemies  = (level==1)? 10   : (level==2)? 12   : 15;    // límite por nivel
    
    uint8_t player_x = 3;
    game_clear_entities();

    uint16_t t_enemy=0, t_pshot=0, t_spawn=0, t_input=0;
    uint8_t spawned = 0;

    // primer enemigo
    if(spawn_enemy()) spawned++;

    while(1){
        // Ejecucion tras lectura de botones
        if(t_input >= 70){
            if(btn_left()  && player_x<7) player_x++;
            if(btn_right() && player_x>0) player_x--;
            if(btn_fire()  && !pshot.act){ pshot.act=1; pshot.x=player_x; pshot.y=6; }
            t_input=0;
        }

        // Bala del jugador
        if(t_pshot >= pshot_step_ms){
            t_pshot=0;
            if(pshot.act){
                if(pshot.y==0){ pshot.act=0; }
                else{
                    uint8_t hit=0;
                    for(uint8_t i=0;i<MAX_ENEMIES;i++){
                        if(enem[i].act && enem[i].y==pshot.y){
                            if(pshot.x==enem[i].x || pshot.x==(enem[i].x+1)){
                                enem[i].act=0; hit=1; break;
                            }
                        }
                    }
                    if(hit) pshot.act=0; else pshot.y--;
                }
            }
        }

        // Descenso de enemigos
        if(t_enemy >= enemy_step_ms){
            t_enemy=0;
            for(uint8_t i=0;i<MAX_ENEMIES;i++){
                if(enem[i].act && enem[i].y < 7) enem[i].y++;
            }
        }

        // Spawn hasta el límite del nivel
        if(t_spawn >= spawn_gap_ms){
            t_spawn=0;
            if(spawned < target_enemies){
                if(spawn_enemy()) spawned++;
            }
        }

        // Derrota si enemigo llega a y>=6 (una fila sobre el jugador)
        for(uint8_t i=0;i<MAX_ENEMIES;i++)
            if(enem[i].act && enem[i].y>=6) return 0;

        // Victoria: todos los spawns hechos y ninguno vivo
        if(spawned==target_enemies && live_count()==0) return 1;

        // Imagen y refresh (10 ms)
        draw_scene(player_x);
        show_ms(10);

        // ticks para actualizacion de matriz
        t_enemy +=10; t_pshot+=10; t_spawn+=10; t_input+=10;
    }
}

// --------------- Menú 1-2-3 ----------------
void blit_digit(uint8_t level){ if(level==1) blit(DIG1); 
  else if(level==2) blit(DIG2); 
  else blit(DIG3); }

uint8_t select_level(void){
    uint8_t level=1, pL=0, pR=0, pA=0;
    for(;;){
        blit_digit(level);
        show_ms(10);
        uint8_t bL=btn_left(), bR=btn_right(), bA=btn_fire();
        if(bR && !pR && level<3) level++;
        if(bL && !pL && level>1) level--;
        if(bA && !pA) return level;
        pL=bL; pR=bR; pA=bA;
    }
}
...
int main(void){
    io_init();
    while(1){
        uint8_t lvl = select_level();
        fb_clear(); show_ms(120);
        uint8_t win = run_level(lvl);
        fb_clear();
        if(win){ pic_victory(); blit(IMG_SMILE); show_ms(1700); }
        else   { pic_death();   blit(IMG_X);     show_ms(1700); }
        // vuelve al menú
    }
}